 ...
 */
export function mapCategoryToDomain(category?: string, title?: string): IssueDomain {
  const c = `${category || ""} ${title || ""}`.toLowerCase();
  if (c.includes("water") || c.includes("handpump") || c.includes("plumb") || c.includes("fluoride") || c.includes("well")) {
    return "Water Resources";
  }
  if (c.includes("agri") || c.includes("farm") || c.includes("crop") || c.includes("irrigation") || c.includes("kisan") || c.includes("drought")) {
    return "Agriculture";
  }
  if (c.includes("health") || c.includes("medic") || c.includes("doctor") || c.includes("phc") || c.includes("malnutrition") || c.includes("stunt")) {
    return "Healthcare";
  }
  if (c.includes("edu") || c.includes("school") || c.includes("college") || c.includes("class") || c.includes("student")) {
    return "Education";
  }
  if (c.includes("mine") || c.includes("mining") || c.includes("coal") || c.includes("dust") || c.includes("reclamation") || c.includes("forest") || c.includes("tree")) {
    return "Environment";
  }
  if (c.includes("pothole") || c.includes("road") || c.includes("street") || c.includes("traffic") || c.includes("bridge") || c.includes("drain")) {
    return "Urban Development";
  }
  if (c.includes("power") || c.includes("electric") || c.includes("light") || c.includes("grid")) {
    return "Energy";
  }
  if (c.includes("sanitat") || c.includes("toilet") || c.includes("garbage") || c.includes("waste")) {
    return "Sanitation";
  }
  if (c.includes("livelihood") || c.includes("tussar") || c.includes("lac") || c.includes("artisan") || c.includes("shg")) {
 ...
 */
export function extractDistrict(locationStr?: string, coordinates?: { lat: number; lng: number } | null): string {
  ...
  const JHARKHAND_NAMES = [
    ...
    "Giridih", "Palamu", "Garhwa", "Latehar", "Ramgarh", "Dumka", "Godda",
    "Sahibganj", "Seraikela Kharsawan", "Khunti", "Lohardaga", "West Singhbhum",
    "Gumla", "Pakur", "Jamtara", "Koderma", "Chatra", "Simdega"
  ];
...
export function normalizeReportToIssue(doc: any): any {
  const idStr = doc._id ? doc._id.toString() : doc.id || new mongoose.Types.ObjectId().toString();
  const trackingCode = doc.id && doc.id.startsWith("CR-") ? doc.id : `CR-JH-${idStr.slice(-6).toUpperCase()}`;
...
export function mapToFlutterReport(issue: any): PhoneAppReport {
  ...
  const tracking = issue.trackingCode || `CR-JH-${idStr.slice(-6).toUpperCase()}`;
  ...
  return {
    ...
    category: (issue.domain || issue.category || "urban_development").toLowerCase().replace(/\s+/g, "_"),
    ...
    coordinates: issue.location?.lat ? issue.location : { lat: 23.3441, lng: 85.3096 },
...
export async function syncReportsToIssues(): Promise<void> {
  try {
    ...
    for (const report of rawReports) {
      ...
      const trackingCode = report.id && report.id.startsWith("CR-") ? report.id : `CR-JH-${idStr.slice(-6).toUpperCase()}`;
