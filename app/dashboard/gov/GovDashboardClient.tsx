  ...
  return (
    <div
      className={`bg-civic-surface border ${
        accent
          ? "border-civic-secondary border-t-4 border-t-civic-secondary"
          : "border-civic-border border-t-4 border-t-civic-primary"
    ...
    >
      ...
      <div className="text-xs font-bold text-civic-textDark mt-1.5">
        {label}
      </div>
      {sub && (
        <div className="text-[11px] text-civic-textMuted mt-0.5">{sub}</div>
      )}
    </div>
  );
}
...
function ReviewQueue() {
  ...
  const [action, setAction] = useState<
    "mark_duplicate" | "confirm_distinct" | ""
  >("");
  ...
  return (
    <div>
      ...
      {loading && (
        <p style={{ fontSize: 13, color: "#64748b" }}>Loading queue…</p>
      )}
      ...
      >
            ...
            <tbody>
              {issues.map((issue) => (
                <tr
                  ...
                  style={{
                    background:
                      selected?._id === issue._id ? "#eff6ff" : "#fff",
        ...
        {selected && (
          ...
          >
            ...
            {selected.similarIssueIds &&
              selected.similarIssueIds.length > 0 && (
            ...
            <div style={{ borderTop: "1px solid #e2e8f0", paddingTop: 12 }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 10 }}>
                <button
                  ...
                  style={{
                    ...
                    padding: "8px 0",
                    background:
                      action === "confirm_distinct" ? "#065f46" : "#f0fdf4",
                ...
                <button
                  ...
                  style={{
                    ...
                    padding: "8px 0",
                    background:
                      action === "mark_duplicate" ? "#7c3aed" : "#faf5ff",
              ...
              {action && (
                <button
                  ...
                  disabled={
                    submitting || (action === "mark_duplicate" && !duplicateOf)
                  }
                  style={{
                    width: "100%",
                    padding: "9px 0",
...
const OVERRIDE_JUSTIFICATION_OPTIONS = [
  {
    ...
    description:
      "Standard open-bidding window expired with 0 qualifying proposals. Activating mandatory direct allocation ladder to prevent grievance abandonment.",
  ...
  },
  {
    ...
    description:
      "Urgent public health, toxic contamination, structural collapse, or environmental emergency requiring immediate institutional mobilization.",
  ...
  },
  {
    ...
    description:
      "Departmental directive assigning the issue based on verified specialized laboratory facilities, patent expertise, or district proximity.",
...
function AllocationOverride() {
  ...
  const [selectedIssue, setSelectedIssue] =
    useState<OverrideIssueOption | null>(null);
  const [selectedCollege, setSelectedCollege] =
    useState<OverrideCollegeOption | null>(null);
  ...
  useEffect(() => {
    function handleClickOutside(e: MouseEvent) {
      if (
        ...
        !issueDropdownRef.current.contains(e.target as Node)
      ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedIssue) {
      setFeedback({
        type: "error",
        message: "Please select a target civic challenge from the dropdown.",
      });
      return;
    }
    if (!selectedCollege) {
      setFeedback({
        type: "error",
        message: "Please select an accredited academic institution.",
      });
      return;
    }
    if (!reason.trim()) {
      setFeedback({
        type: "error",
        message: "Please select an administrative override justification.",
      });
      return;
    }
    ...
    try {
      ...
      if (!res.ok || !data.success) {
        throw new Error(
          data.error || "Failed to execute institutional override allocation.",
        );
      }
      ...
      setFeedback({
        ...
        message:
          data.message ||
          `Successfully allocated ${selectedIssue.trackingCode} to ${selectedCollege.name}.`,
        details: {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-xs text-civic-textMuted -mt-2">
          Statutory executive mechanism to bypass open marketplace bidding and
          directly allocate grassroots grievances to accredited HEIs under
          disaster directives or escalation triggers.
        </p>
      </div>
        ...
        >
          ...
          <div className="flex-1 text-xs">
            <div className="font-bold text-sm">
              {feedback.type === "success"
                ? "Allocation Override Executed Successfully"
                : "Action Required"}
            </div>
      ...
      <form
        ...
        className="bg-civic-surface border border-civic-border rounded-xl shadow-xs p-6 space-y-6"
      >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="space-y-2">
              ...
              >
                {selectedIssue ? (
                  ...
                  <span className="text-xs text-slate-400">
                    {loadingData
                      ? "Loading open issues..."
                      : "Select or search open grievance..."}
                  </span>
                )}
                  ...
                  <div className="overflow-y-auto divide-y divide-slate-100 flex-1">
                    {filteredIssues.length === 0 ? (
                      ...
                      filteredIssues.map((issue) => {
                        ...
                        const isDisaster =
                          issue.urgencyTrack === "DISASTER_FAST_TRACK";
            ...
            {selectedIssue && (
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs space-y-1 animate-fadeIn">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-civic-primary font-mono">
                    {selectedIssue.trackingCode}
                  </span>
                ...
                <p className="font-medium text-civic-textDark leading-tight">
                  {selectedIssue.title}
                </p>
          ...
          <div className="space-y-2">
                ...
                {selectedCollege ? (
                  ...
                  <span className="text-xs text-slate-400">
                    {loadingData
                      ...
                      : "Select or search accredited university / college..."}
                  </span>
                )}
                <div className="flex items-center gap-1 text-slate-400 shrink-0">
                  {selectedCollege && (
                  ...
                  <div className="overflow-y-auto divide-y divide-slate-100 flex-1">
                    {filteredColleges.length === 0 ? (
                      ...
                      filteredColleges.map((college) => {
                        ...
                        const facilitiesList = (college.facilities || []).map(
            ...
            {selectedCollege && (
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-lg text-xs space-y-1 animate-fadeIn">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-civic-textDark">
                    {selectedCollege.name}
                  </span>
                ...
                <div className="flex items-center gap-3 text-[11px] text-civic-textMuted pt-1 border-t border-slate-200/60">
                  ...
                  {selectedCollege.reputationScore && (
                    <span>
                      Rating:{" "}
                      <strong className="text-civic-textDark">
                        {selectedCollege.reputationScore}/5.0
                      </strong>
                    </span>
                  )}
                </div>
        ...
        <div className="border-t border-civic-border pt-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="text-xs text-civic-textMuted flex items-center gap-1.5">
            ...
            <span>
              Executing override immediately updates issue status to{" "}
              <strong className="text-civic-textDark">
                DIRECTLY_ALLOCATED
              </strong>{" "}
              and dispatches high-priority dispatch notifications.
            </span>
          </div>
...
function CollegeVerification() {
  ...
  const toggle = async (id: string) => {
    setToggling(id);
    const res = await fetch(`/api/gov/colleges/${id}/verify`, {
      method: "PATCH",
    });
    if (res.ok) {
      const d = await res.json();
  ...
  return (
    <div>
      ...
      {loading && (
        <p style={{ fontSize: 13, color: "#64748b" }}>Loading colleges…</p>
      )}
...
function AnalyticsDashboard() {
  ...
  if (loading)
    return <p style={{ fontSize: 13, color: "#64748b" }}>Loading analytics…</p>;
  if (!data)
    return (
    ...
    <div>
      ...
      >
        ...
        <StatCard
          ...
          value={data.summary.totalProposals}
        />
        <StatCard
          label="Industry Pledges Funded"
          value={data.summary.totalFunded}
          accent
        />
      </div>
            ...
            <JharkhandMap
              ...
              districtData={Object.fromEntries(
                data.issuesByDistrict
                  ...
                  .map((d) => [
                    d.district,
                    { count: d.issues, density: Math.min(100, d.issues * 4) },
                  ]),
              )}
            />
          </div>
      ...
      <div
        style={{ border: "1px solid #dde2ea", padding: 16, background: "#fff" }}
      >
        <p
          style={{
            fontFamily: "Georgia, serif",
