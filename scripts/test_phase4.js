async function testPhase4() {
  ...
  console.log(
    "   CivicResolve Phase 4 End-to-End API & Dashboard Verification   ",
  );
  console.log(
    "==================================================================",
  );
  ...
  const sampleDist =
    summaryData.data["Lohardaga"] || summaryData.data["Ranchi"];
  ...
  const newIssuePayload = {
    ...
    description:
      "Monsoon floods washed away the approach slab of the Kuru-Senha road culvert, cutting off 8 agricultural villages from the block market.",
    domain: "Agriculture",
    district: "Lohardaga",
  ...
  console.log(
    `-> HTTP ${createRes.status} | Tracking Code: ${createData.trackingCode}`,
  );
  console.log(
    `-> Duplicate Flagged: ${createData.isDuplicateFlagged} | Status: ${createData.data?.status}`,
  );
  console.log(
    `-> Repeat Reporter Index: ${createData.submissionIndexForMobile}`,
  );
  ...
  console.log(
    `-> HTTP ${dupRes.status} | Tracking Code: ${dupData.trackingCode}`,
  );
  console.log(
    `-> Duplicate Flagged: ${dupData.isDuplicateFlagged} (Expected: true)`,
  );
  console.log(
    `-> Status Assigned: ${dupData.data?.status} (Expected: Under_Review)`,
  );
  console.log(
    `-> Similar Issue IDs count: ${dupData.data?.similarIssueIds?.length}`,
  );
  console.log(
    `-> Repeat Reporter Index: ${dupData.submissionIndexForMobile} (Expected: 2)`,
  );
  ...
  console.log(
    `-> HTTP ${detailRes.status} | District: ${detailData.district.name}`,
  );
  ...
  detailData.issues.slice(0, 2).forEach((iss, i) => {
    console.log(
      `   ${i + 1}. [${iss.trackingCode}] ${iss.title} (${iss.status})`,
    );
  });
